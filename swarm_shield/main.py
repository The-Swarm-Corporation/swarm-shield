...
class SwarmShield:
    ...
    def __init__(
        ...
        self.storage_path = Path(
            storage_path or "swarm_shield_storage"
        )
        ...
        logger.info(
            f"SwarmShield initialized with {encryption_strength.value} encryption"
        )
    ...
    def delete_conversation(self, conversation_id: str) -> None:
        ...
        try:
            with self._conv_lock:
                ...
                # Remove from disk
                conv_path = (
                    self.storage_path / f"{conversation_id}.conv"
                )
                if conv_path.exists():
                    conv_path.unlink()
            ...
            with self._conv_lock:
                for conv_id, conv_data in self._conversations.items():
                    ...
                    for msg in conv_data["messages"]:
                        ...
                        agent, content = self.retrieve_message(
                            msg["data"]
                        )
                        timestamp = datetime.fromisoformat(
                            msg["timestamp"]
                        ...
                        if (
                            text
                            and text.lower() not in content.lower()
                        ):
                            continue
            ...
            with self._conv_lock:
                ...
                for conv_data in self._conversations.values():
                    ...
                    for msg in conv_data["messages"]:
                        ...
                        if agent == agent_name:
                            ...
                            if (
                                not stats["first_message"]
                                or timestamp < stats["first_message"]
                            ):
                                stats["first_message"] = timestamp
                            if (
                                not stats["last_message"]
                                or timestamp > stats["last_message"]
                            ):
                ...
                if stats["total_messages"] > 0:
                    stats["avg_message_length"] = (
                        total_length / stats["total_messages"]
                    )
        ...
        try:
            with self._conv_lock:
                ...
                for msg in conv_data["messages"]:
                    ...
                    if (
                        not summary["last_message"]
                        or timestamp > summary["last_message"]
                    ):
    ...
    def export_conversation(
        ...
        try:
            ...
            if format == "json":
                ...
                for agent, message, timestamp in messages:
                    export_text.append(
                        f"[{timestamp.isoformat()}] {agent}: {message}"
                    )
    ...
    def backup_conversations(
        ...
        try:
            ...
            backup_path = Path(
                backup_dir or self.storage_path / "backups"
            )
